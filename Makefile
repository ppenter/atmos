...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(ATMOS_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(ATMOS_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(ATMOS_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(ATMOS_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(ATMOS_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\atmos\Z atmosd/node "./atmosd testnet --v 4 -o /atmos --keyring-backend=test --ip-addresses atmosdnode0,atmosdnode1,atmosdnode2,atmosdnode3"
	docker-compose up -d
else
	mkdir -p localnet-setup
	@$(MAKE) localnet-build

	if ! [ -f localnet-setup/node0/$(ATMOS_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/localnet-setup:/atmos:Z atmosd/node "./atmosd testnet --v 4 -o /atmos --keyring-backend=test --ip-addresses atmosdnode0,atmosdnode1,atmosdnode2,atmosdnode3"; fi
	docker-compose up -d
endif
